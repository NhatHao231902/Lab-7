...
print 'Tuoi nhan vien: ' + convert(nvarchar, dbo.fn_TuoiNV('007'))
----Nhập vào Manv cho biết số lượng đề án nhân viên này đã tham gia
SELECT MA_NVIEN, COUNT(MADA) FROM PHANCONG
GROUP BY MA_NVIEN

SELECT COUNT(MADA) FROM PHANCONG WHERE MA_NVIEN = '007'
...
print ' so du an nhan vien da lam la: ' + convert(varchar, dbo.fn_DemDeAnNV('007'))
...
----Truyền tham số đầu vào là tên phòng, tính mức lương trung bình của phòng đó, Cho biết họ tên nhân viên (HONV, TENLOT, TENNV) có mức lương trên mức lương trung bìnhcủa phòng đó.
